      ...
      <div className="px-4 flex flex-row">
        <a href="/">
          <button className="group relative overflow-hidden cursor-pointer flex flex-row">
            <Image onMouseEnter={() => setIsEyeIconHovered(true)} onMouseLeave={() => setIsEyeIconHovered(false)} className="w-auto h-auto px-2 py-0.5 rounded-lg rounded-r-none rounded-b-none border-t-4 border-l-4 border-foreground border-double pixelated" src={isEyeIconHovered ? "/icon_closed.png" : "/icon.png"} alt="A 1-bit (black & white) pixel art icon of an eye, wide-open. The site logo. The eye closes when hovered over by the mouse pointer." width={0} height={0} unoptimized loading="eager" />
