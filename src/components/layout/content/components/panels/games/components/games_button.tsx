...
export default function GamesButton({ onClick, state, date, title, genres, tagline, src, alt, imgW, imgH }: GamesButtonProps) {
  ...
  return (
    <div className="relative overflow-hidden shrink-0 h-fit min-h-44 flex flex-row">
      <button onClick={onClick} className="cursor-pointer group relative overflow-hidden w-50 min-w-1/2 px-4 py-2 rounded-lg rounded-r-none border-4 border-r-0 border-dark border-double bg-light text-dark flex flex-col items-start gap-2">
      ...
      <button onClick={() => openModal(`${title} thumbnail`, <Image className="object-contain pixelated" src={src} alt={alt} fill unoptimized />)} className="cursor-pointer group relative flex-1 overflow-hidden rounded-lg rounded-l-none border-4 border-l-0 border-dark border-double">
        ...
        <Image className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 max-w-none max-h-none group-hover:scale-150 transition-transform duration-300 ease-out pixelated" src={src} alt={alt} width={imgW} height={imgH} unoptimized loading="eager" />
      </button>
    </div>
  );
}
