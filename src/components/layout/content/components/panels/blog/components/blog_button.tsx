...
export default function BlogButton({onClick, date, title, tagline, wordCount, src, alt, imgW, imgH}: BlogButtonProps) {
  ...
  const min = Math.ceil(wordCount / AVG_NONFICTION_WORDS_PER_MIN);

  return (
    <div
      className="cursor-pointer relative overflow-hidden shrink-0 h-fit min-h-31.25 flex flex-row">
      <button
        onClick={onClick}
        className="cursor-pointer group relative overflow-hidden w-50 min-w-1/2 px-4 py-2 rounded-lg rounded-r-none border-4 border-r-0 border-dark border-double bg-light text-dark flex flex-col items-start gap-1">
        <HoveredDither />
