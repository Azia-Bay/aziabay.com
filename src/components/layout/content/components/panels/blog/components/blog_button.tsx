...
export default function BlogButton({onClick, date, title, tagline, wordCount, src, alt}: BlogButtonProps) {
  ...
  return (
    <div className="relative overflow-hidden cursor-pointer shrink-0 min-h-31.25 flex flex-row">
      ...
      <button onClick={() => openModal("The Creation of aziabay.com thumbnail", <Image className="max-w-full max-h-full w-auto h-auto pixelated" src={src} alt={alt} width={0} height={0} unoptimized />)} className="cursor-pointer group relative flex-1 overflow-hidden rounded-lg rounded-l-none border-4 border-l-0 border-background border-double">
        ...
        <Image className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-auto h-auto max-w-none group-hover:scale-150 transition-transform duration-300 ease-out pixelated" src={src} alt={alt} width={0} height={0} unoptimized />
      </button>
    </div>
  );
}
